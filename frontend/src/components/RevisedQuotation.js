...
const RevisedQuotation = ({ match }) => {
    ...
    const userTemplate = { itemname: "", itemdesc: "", itemsize: "", itemuom: "", itemqty: "", itemprice: "", first: "", second: "", third: "", fourth: "", fifth: "", percentagesign: "" }
    ...
    useEffect(() => {
        ...
        if (getInvoice && getInvoice._id !== invoiceId) {
            ...
            setPriceBasis(getInvoice.pricebasis)
            setPayment(getInvoice.payment)
            setDelivery(getInvoice.delivery)
            setMTC(getInvoice.mtc)
            setPackaging(getInvoice.packaging)
            setWarranty(getInvoice.warranty)
        }
        if (error) {
            alert.error(error);
        ...
        if (success) {
            alert.success('Invoice Revised Successfully!');
            history.push('/invoice-list')
            dispatch({
                type: CREATE_INVOICE_RESET
            })
        }
    }, [dispatch, error, getInvoice, alert, success, invoiceId, history])
    return (
        <Fragment>
            ...
            <form>
                <div className='calculator-boxes'>
                    ...
                    <div>
                        ...
                        <input ref={refs.sum} onChange={onCalculator} readOnly defaultValue={vars.sum} id="sum" name="sum" type="number" />
                    </div>
                </div>
            </form>
                    ...
                    loading ? <Loader /> :
                        <Fragment>
                            <div className='breadcrumb'>
                                <p><Link to='/invoice-list'>Quotation List</Link> <span><RightIcon /></span> <span>Revised Quotation</span></p>
                            </div>
                            <h1 className='text-center head-text'>Revis<span>ed Quo</span>tation</h1>
                            <MetaData title={`${getInvoice.companyname} - Revised Quotation`} />
                            <form onSubmit={handleInvoiceUpdate}>
                                <div className='Header-inputs'>
                                    <div>
                                        <label htmlFor="">Email -</label>
                                        <input className='mb-4' name='email' value={email} onChange={(e) => setEmail(e.target.value)} type="text" placeholder='To *' /> <br />
                                ...
                                <div className='project-Input-Head'>
                                    ...
                                    {/* Extra Field for Terms & condition */}
                                    <Button type='submit' variant="contained" className='submit-button mt-5'>Revised Quotation &nbsp; <SendIcon /></Button>
                                </div>
                            </form>
                        </Fragment>
                }
...
export default RevisedQuotation