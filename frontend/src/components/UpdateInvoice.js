...
const UpdateInvoice = ({ match }) => {
    ...
    const termsChange = (e, index) => {
        ...
        setTerms(addTerms)
        const addTerm = terms.map((tData, i) => index == i ? Object.assign(tData, { [e.target.name]: e.target.value }) : tData)
        setTermsData(addTerm)
    }
    ...
    const handleInvoiceUpdate = (e) => {
        e.preventDefault();
        dispatch(updateInvoice(invoiceId, { companyname, revisionnumber, customerrefno, projectname, kindatt, email, contact, validity, products, items, pricebasis, payment, delivery, mtc, warranty, packaging, terms, termsData }));
    }
